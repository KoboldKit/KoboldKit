...
typedef enum
{
	KTIvarTypeUnknown = 0,
	KTIvarTypeChar,
	KTIvarTypeUnsignedChar,
	KTIvarTypeInt,
	KTIvarTypeUnsignedInt,
	KTIvarTypeDouble,
	KTIvarTypeFloat,
	KTIvarTypePoint,
	KTIvarTypeSize,
...
@property (nonatomic, readonly) KTIvarType type;
...
@interface KTIvarSetter : NSObject
{
	@private
	Class _class;
	Ivar* _ivars;
...
/** Sets the ivars in the target if their key in the dictionary matches the ivar name and type. The dictionary must have NSString as keys
   and either NSString or KKMutableNumber as values. The target's class must match the class the KTIvarSetter object was initialized with. */
